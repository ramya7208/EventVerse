  ...
  return (
    <div onClick={() => onGo(event)}
      className={`bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border cursor-pointer hover:shadow-lg transition-all duration-300 group ${
        isUpcoming ? "border-2 border-violet-300 dark:border-violet-700" : "border-gray-100 dark:border-gray-800 hover:border-violet-200 dark:hover:border-violet-800"
      }`}>
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-2 left-2">
          <span className="text-xs px-2.5 py-1 rounded-full font-bold bg-violet-50 dark:bg-violet-900/40 text-violet-800 dark:text-violet-300">Workshop</span>
        </div>
      ...
      <div className="p-4">
        ...
        <h3 className="font-bold text-gray-900 dark:text-white text-base mb-1 group-hover:text-violet-600 dark:group-hover:text-violet-400 transition line-clamp-1">{event.title}</h3>
        ...
        {isUpcoming && (
          <div className="flex items-center justify-between">
            ...
            <button onClick={e => { e.stopPropagation(); onGo(event); }}
              className="text-xs px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-bold transition hover:scale-105">
              Register →
            </button>
          </div>
        )}
...
export default function Workshops() {
  ...
  const events = EVENTS.filter(e => e.category === "workshop");
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-2">
        <div className="w-3 h-8 bg-violet-500 rounded-full" />
        <h1 className="text-2xl font-black text-gray-900 dark:text-white">Workshops</h1>
      </div>
      <p className="text-gray-400 dark:text-gray-500 text-sm mb-6 ml-6">Hands-on sessions to sharpen your skills</p>
