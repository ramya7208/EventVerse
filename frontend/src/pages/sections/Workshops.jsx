    ...
    <div className={`bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border transition-all ${
      ...
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-2 left-2">
          <span className="text-xs px-2 py-1 rounded-full font-medium bg-purple-50 dark:bg-purple-900/40 text-purple-800 dark:text-purple-300">Workshop</span>
        </div>
      ...
      <div className="p-4">
        ...
        {isUpcoming && (
          <div className="flex items-center justify-between mt-2">
            ...
            <button onClick={() => onRegister(event)} className="text-xs px-4 py-2 rounded-lg font-medium text-white bg-purple-600 hover:bg-purple-700 transition">
              Register Now
            </button>
          </div>
        )}
...
export default function Workshops() {
  ...
  const handleRegister = (event) => {
    navigate("/event", { state: { title: event.title, date: event.date, domain: "workshop", description: event.description, image: event.image, seats: event.seats } });
  };

  return (
    <div>
      <div className="flex items-center gap-3 mb-2">
        <div className="w-3 h-8 bg-purple-500 rounded-full" />
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Workshops</h1>
      </div>
