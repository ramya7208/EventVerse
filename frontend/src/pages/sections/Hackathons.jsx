...
// CASE 4+6: Register navigates to /event/:id (new page)
...
function CollegeLogo({ college }) {
  ...
  return (
    <div className="w-5 h-5 rounded bg-white dark:bg-gray-700 border border-gray-100 dark:border-gray-600 flex items-center justify-center overflow-hidden flex-shrink-0">
      {!err ? (
        <img src={college.logo} alt={college.shortName} className="w-4 h-4 object-contain"
          onError={() => setErr(true)} />
      ) : (
        <span className="text-xs font-bold" style={{ color: college.color, fontSize: "8px" }}>{college.initials}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div
      ...
      className={`bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border cursor-pointer hover:shadow-lg transition-all duration-300 group ${
        isUpcoming ? "border-2 border-red-300 dark:border-red-700" : "border-gray-100 dark:border-gray-800 hover:border-red-200 dark:hover:border-red-800"
      }`}>
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-2 left-2">
          <span className="text-xs px-2.5 py-1 rounded-full font-bold bg-red-50 dark:bg-red-900/40 text-red-800 dark:text-red-300">Hackathon</span>
        </div>
      ...
      <div className="p-4">
        ...
        <h3 className="font-bold text-gray-900 dark:text-white text-base mb-1 group-hover:text-red-600 dark:group-hover:text-red-400 transition line-clamp-1">{event.title}</h3>
        ...
        {isUpcoming && (
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold transition hover:scale-105">
              Register →
            </button>
          </div>
        )}
...
export default function Hackathons() {
  ...
  // ✅ CASE 4+6 FIX: navigate to /event/:id as a NEW PAGE
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-2xl font-black text-gray-900 dark:text-white">Hackathons</h1>
      </div>
